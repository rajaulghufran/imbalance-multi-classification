...
from util import convert_df, create_vocab_df, delete_state, delete_states, filter_dataframe, filter_dataframe_single_column, get_term_doc_freq_df, init_state, instantiate_classification, read_dataset
...
if clf.is_fitted():
    ...
    st.dataframe(
        {
            k.split("__")[-1]: ", ".join(sorted(v)) if k == "pos_filter__pos" else str(v)
            for k, v in model_attrs.items()
            if k not in ["stopword_removal__stopwords", "tfidfvectorizer__vocabulary", "tfidfvectorizer__stop_words"]
        },
    ...
    if "testing.test.succeed" in st.session_state:
        ...
        st.title("Results")
        st.subheader("Predictions have been added to the dataset!")
        ...
        if "testing.y_test" in st.session_state:
            if (
                ...
                with col:
                    cm = ConfusionMatrixDisplay.from_predictions(st.session_state["testing.y_test"], st.session_state["testing.y_pred"], normalize="true", cmap="YlGn")
    ...
    st.markdown("### Model is not fitted.")
